...
def client_thread_func(sock, left_limb, right_limb):
    ...
    while True:        
        ...
        #print('cmd_bytes = ', len(cmd_bytes), ', content = ', cmd_bytes)
        ...
        if not data:
           break
        #rospy.loginfo("from connected user: " + str(data))
        #data = input(' -> ')
        ...
        if data == "GetPosR":
            ...
            rospy.loginfo(" >>> right l = ", l)
            sock.send(struct.pack('<I', l))
            rospy.loginfo("Sending Right Arm Data")
            sock.send(json_right.encode("ascii"))  # send data to the client
        if data == "SetPosL":    
            l_bytes = sock.recv(4)
            l = struct.unpack('<I', l_bytes)
            rospy.loginfo("left l = ", l[0])
            text_data = sock.recv(l[0]).decode("ascii")
            #rospy.loginfo("text_data = ", text_data)
            left_dict = json.loads(text_data)
            rospy.loginfo("left_dict = ", left_dict)
            sock.send("ACK".encode("ascii"))
            set_both_arms += 1
        ...
        if data == "SetPosR":    
            l_bytes = sock.recv(4)
            l = struct.unpack('<I', l_bytes)
            rospy.loginfo("right l = ", l[0])
            text_data = sock.recv(l[0]).decode("ascii")
            #rospy.loginfo("text_data = ", text_data)
            right_dict = json.loads(text_data)
            rospy.loginfo("right_dict = ", right_dict)
            sock.send("ACK".encode("ascii"))
            set_both_arms += 1
